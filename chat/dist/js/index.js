...
var ChatModule = {
    ...
    event: function event() {
        ...
        self.domLogin.addEventListener('click', function () {
            var sNick = self.domNickName.value;
            if (!sNick) {
                alert('名字还是要填的!');
                return false;
            }
        ...
        self.domMsgSend.addEventListener('click', function () {
            var sMsg = self.domMsgText.value;
            if (!sMsg) {
                alert('消息还是要填的!');
                return false;
            }
    ...
    socketConn: function socketConn(rs) {
        var self = this;
    ...
    wsSend: function wsSend(rs) {
        var self = this;
        self.ws.send(JSON.stringify(rs));
    },
    appendMsg: function appendMsg(rs) {
        var self = this,
        ...
        if (rs.type == 'Send') {
            if (rs.nick == self.sNick) {
                _shtml += ' <div class="msg"> ' + '<div class="ico right"> ' + '<div class="img"> </div> ' + '<div class="nick">' + rs.nick + '</div> ' + '</div> ' + '<div class="text right">' + rs.msg + '</div> ' + '</div>';
            } else {
                _shtml += ' <div class="msg"> ' + '<div class="ico"> ' + '<div class="img"> </div> ' + '<div class="nick">' + rs.nick + '</div> ' + '</div> ' + '<div class="text">' + rs.msg + '</div> ' + '</div>';
            }
        }
